...
namespace data {
  ...
  public:
    ...
    uint32_t Get_start_index() const {
      return reinterpret_cast<const uint32_t &>(_header[Index::HorizontalAngle]);
    }

    void Set_end_index(uint32_t index) {
      std::memcpy(&_header[Index::HorizontalAngle], &index, sizeof(uint32_t));
    }
	// modify --!>
